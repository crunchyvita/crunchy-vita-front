        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-grow bg-white rounded-lg shadow-sm p-6">
            ...
            {isEmpty ? (
              ...
              <div className="divide-y divide-gray-100">
                {cartItems.map((item) => {
                  // For packages, collect all product images
                  const isPackage = item.type === 'package';
                  let productImages = [];
                  
                  if (isPackage && item.selectedProducts?.length > 0) {
                    productImages = item.selectedProducts.map(sp => {
                    ...
                    // Fallback to item.image if no product images found
                    if (productImages.length === 0 && item.image) {
                      productImages = [item.image];
                    }
                  } else {
                    // Regular product
                    if (item.image) productImages = [item.image];
                  }
                  
                  return (
                  <div key={item._id} className="py-6 flex items-center gap-4">
                    {/* Product Image(s) */}
                    {isPackage && productImages.length > 0 ? (
                      <div className="flex gap-1 flex-shrink-0">
                        {productImages.slice(0, 3).map((img, idx) => (
                          <div key={idx} className="w-16 h-20 bg-gray-100 rounded-md overflow-hidden">
                            <img src={img} alt={`Product ${idx + 1}`} className="w-full h-full object-cover" />
                          </div>
                        ))}
                        {productImages.length > 3 && (
                          <div className="w-16 h-20 bg-gray-900/80 rounded-md flex items-center justify-center">
                            <span className="text-white text-xs font-bold">+{productImages.length - 3}</span>
                          </div>
                        )}
                      </div>
                    ) : (
                      <div className="w-24 h-24 bg-gray-100 rounded-md overflow-hidden flex-shrink-0">
                        {productImages[0] ? (
                          <img src={productImages[0]} alt={item.name} className="w-full h-full object-cover" />
                        ) : (
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-sm text-gray-500">{t('products.price')}: €{item.price.toFixed(2)}</p>
                    </div>
          ...
          <aside className="lg:w-80">
            <div className="bg-white rounded-lg shadow-sm p-6 sticky top-8">
              <h2 className="text-xl font-black text-[#556822] mb-6 font-[agrandir]">
                {t('summary.title')}
              </h2>
